$(document).ready(() => {
  ...
  蛾儿雪柳黄金缕。笑语盈盈暗香去。
  众里寻他千百度。蓦然回首，那人却在，灯火阑珊处。`, `一剪梅
  李清照
  红藕香残玉簟秋。轻解罗裳，独上兰舟。
  云中谁寄锦书来？雁字回时，月满西楼。

  花自飘零水自流。一种相思，两处闲愁。
  此情无计可消除，才下眉头，却上心头。`, `虞美人
  李煜
  春花秋月何时了，往事知多少？
  小楼昨夜又东风，故国不堪回首月明中。

  雕阑玉砌应犹在，只是朱颜改。
  问君能有几多愁？恰似一江春水向东流。`, `雨霖铃
  柳永
  寒蝉凄切，对长亭晚，骤雨初歇。
  都门帐饮无绪，留恋处，兰舟催发。
  执手相看泪眼，竟无语凝噎。
  ...
  const poemElements = poetry.map((poem) => {
    ...
    for (let i = 2; i < lines.length; i += 1) {
      content += `<p>${lines[i].replace(/ /g, '').replace(/(，|\n|、)/g, '<br>').replace(/(。|？|！)/g, '</p><p>')}</p>`;
    }
    return `<h1>${title}</h1>
  ...
  $('.original').html(
    poemElements[Math.floor(Math.random() * poemElements.length)]
  );
