...
function searchFunc(path, searchId, contentId) {
  $.ajax({
    ...
    success(xmlResponse) {
      ...
      const dataset = $('entry', xmlResponse).map(function () {
        return {
      ...
      $input.on('input', (e) => {
        ...
        setTimeout(() => {
          ...
          str += '</ul>';
          const title = matchCount ? `<h1>找到<em>${value}</em>相关内容${matchCount}个</h1>` : `<h1 class="empty-title">没有找到<em>${value}</em>相关内容</h1>`;
          str = title + str;
          $('#content').css({
            display: 'none'
          });
